...
local Slider = Movement:AddSlider({
  ...
  local Button = Movement:AddButton({
    Name = "Fly",
    Callback = function()
...
-- ESP BUTTON
local Section = Aimbot:AddSection({"ESP"})
...
-- FUN STUFF TAB
local Fun = Window:MakeTab({Name = "Fun Stuff", Icon = "Settings"})
local Section = Fun:AddSection({Name = "Animations + Extras.. (Need to click again if respawned)"})
local Button = Fun:AddButton({
    Name = "Tryhard Anims",
    Callback = function()
        while true do
            wait(1)
...
local Section = Fun:AddSection({Name = "Trash Talk"})
local Button = Fun:AddButton({
    Name = "Trash Talk (J)",
    Callback = function()
